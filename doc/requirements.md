...
Bubble Wrap, Eternal 是一个 `casual` 类型的移动端小游戏：Endless bubble wrap. Tap to pop — pitch-randomized chirp, a satisfying splash, the next row already calling. Every 8th pop drops a deadpan paper fortune that joins Today's Slips wall. AlterU sensory.

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 hsla(var(--hue)、#fff8e6、#efe3bd、#0e1822、rgba(255, 220, 160, 0.18)、#ffd58a、rgba(255,255,255,0.04)、#f3eccf；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 'Inter', system-ui, -apple-system, sans-serif、'IBM Plex Mono', monospace、'IBM Plex Mono', ui-monospace, monospace、'Caveat', cursive，按钮与状态文字保持 12-24 px 的可读范围。
